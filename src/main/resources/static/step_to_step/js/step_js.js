...
function submit_reaults(data) {
    $.ajax({
        ...
        success: function (data) {
            ...
            if(error === 1){
                $("section.grid").append("<blockquote>您的条件太苛刻啦(＃°Д°)，没有适合您的产品，您可以选择适当放款条件。</blockquote>");
            }else {
                if(loosen === 1){
                    $("section.grid").append("<blockquote>您的价格范围太苛刻了，没有适合您的产品。以下是调整价格上限1.5倍后的推荐结果。</blockquote>");
                }
